# WuzAPI - AI Coding Assistant Instructions

## Project Overview
WuzAPI is a multi-tenant WhatsApp REST API service built in Go using the whatsmeow library. It provides a RESTful interface for managing WhatsApp sessions, sending messages, handling groups, and processing webhooks without requiring Puppeteer, Chrome, or Android emulators.

## Architecture
- **Core Library**: whatsmeow for direct WhatsApp WebSocket communication
- **API Layer**: Gorilla Mux router with REST endpoints
- **Data Layer**: PostgreSQL/SQLite with migration system
- **Event System**: Webhook delivery + optional RabbitMQ for global events
- **Storage**: Optional S3 integration for media files
- **Multi-tenancy**: Token-based user isolation

## Key Components
- `main.go`: Server initialization, routing, database setup
- `handlers.go`: REST API endpoint implementations
- `wmiau.go`: WhatsApp client management and event handling
- `db.go`: Database connection and configuration
- `migrations.go`: Database schema management
- `rabbitmq.go`: Global event distribution
- `s3manager.go`: Media storage integration
...
./wuzapi -port=3000 -logtype=json -color=false -admintoken=your_token

# Environment variables override flags
WUZAPI_ADMIN_TOKEN=token DB_USER=user DB_PASSWORD=pass ./wuzapi
```

### Configuration
- **Database**: Set `DB_*` env vars for PostgreSQL, or use SQLite (default)
- **Admin Token**: `WUZAPI_ADMIN_TOKEN` for user management endpoints
- **Timezone**: `TZ=America/New_York` for server operations
- **Webhook Format**: `WEBHOOK_FORMAT=json` (default) or `form`
- **RabbitMQ**: `RABBITMQ_URL` and `RABBITMQ_QUEUE` for global events

### Testing API
- **Swagger UI**: Visit `/api` for interactive API documentation
- **Login Page**: `/login` for QR code scanning
- **Dashboard**: `/dashboard` for session management

## Code Patterns & Conventions

### Authentication
```go
// User authentication via token in Authorization header
...
if adminToken != *adminToken { /* unauthorized */ }
```

### Database Operations
```go
// Use sqlx for queries with named parameters
rows, err := s.db.Query("SELECT id,name FROM users WHERE token=$1", token)

// Always close rows and handle errors
defer rows.Close()
for rows.Next() {
...
### Message Sending
```go
// Base64 data URLs for media
if strings.HasPrefix(data, "data:image/") {
    dataURL, _ := dataurl.DecodeString(data)
...
### Event Handling
```go
// Event types defined in constants.go
supportedEventTypes := []string{"Message", "ReadReceipt", "Presence", "All"}

// Webhook delivery
callHook(webhookURL, map[string]string{
    "jsonData": string(eventJSON),
    "token": userToken,
}, userID)
```
...
s.Respond(w, r, http.StatusBadRequest, errors.New("error message"))

// Logging with zerolog
log.Info().Str("user", userID).Msg("operation completed")
log.Error().Err(err).Str("context", value).Msg("operation failed")
```

## Integration Points

### Webhooks
- **Format**: JSON (default) or form-encoded
- **Events**: Configurable per user (Message, Presence, etc.)
- **Global**: Optional global webhook for all events
- **Authentication**: No auth required (user provides URL)

### RabbitMQ
- **Purpose**: Global event distribution across services
- **Queue**: `whatsapp_events` (configurable)
- **Format**: JSON with userId and instanceName
- **Usage**: Complements webhooks, not replaces them

### S3 Storage
- **Purpose**: Media file storage and delivery
- **Options**: base64, S3, or both
- **Retention**: Configurable days
- **Access**: Public URLs for delivery

### Database
- **Migration System**: Automatic schema updates
- **Multi-DB**: PostgreSQL and SQLite support
- **User Data**: Sessions, webhooks, S3 configs per user
...
## File Organization
- **Root**: Core application files (main.go, handlers.go, etc.)
- **static/**: Web assets (HTML, CSS, JS for dashboard/login)
- **dbdata/**: SQLite database files
- **files/**: User-uploaded media files
- **Environment**: .env file for configuration

## Development Tips
- Use `go mod tidy` after dependency changes
- Update `go.mau.fi/whatsmeow` regularly for WhatsApp protocol changes
- Test with both PostgreSQL and SQLite
- Use admin endpoints for user/session management during development
- Check logs with `-logtype=json` for structured output
